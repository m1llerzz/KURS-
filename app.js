...
(function () {
  ...
  /* ── Данные: бот → кеш → файл ─────────────────────────────
   *
   * Тарифы когда-то лежали в data.js и правились руками. Потом всё
   * собирал бот: обходил ЦБ и bank.uz, считал наценку каждого сервиса к
   * официальному курсу и отдавал готовый набор. Бот остался, но один он
   * больше ничего не держит: данные пересобирает GitHub несколько раз в
   * сутки, а официальный курс приложение спрашивает у ЦБ само.
   *
  ...
  function primenitKursCB(cb) {
    ...
    /* Та же дата — это ПОДТВЕРЖДЕНИЕ, а не «ничего нового».
     *
     * Найдено глазами, в настоящем браузере. Бот молчит, приложение
     * считает по запасу, ЦБ отвечает — и отвечает ровно тем же числом,
     * что в запасе. Раньше слой на этом останавливался, флаг «запас»
     * оставался, и человек читал внизу «курс ЦБ обновить не удалось,
     * цифры ориентировочные» — под цифрой, которую источник только что
     * подтвердил. Запас на то и запас, что в нём может лежать
     * позавчерашнее; сказать «ориентировочно» про сегодняшний
     * официальный курс — соврать в другую сторону. */
    const toZhe = bylo === cb.date;
     ...
     * такого-то». Теперь официальный курс подтверждён источником, и та
     * надпись врёт уже в другую сторону. Свежесть каждого способа при
     * этом считается отдельно, по его `checked_at`: протухшие всё так же
     * не покажутся. */
    dannyeUstareli = null;

    if (toZhe) return true;      // подтвердили — в ряд добавлять нечего
