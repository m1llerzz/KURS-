...
NABORY = {
    ...
    # Дата, которую не разобрать. Наши сборщики такую точку не возьмут
    # вовсе, но она может прийти из кеша истории или из правленого руками
    # запаса — и тогда обе реализации сваливаются в запасной путь: окно
    # считается по количеству точек, недельный сдвиг — от восьмой с конца.
    # Путь этот до сих пор не трогал ни один прогон: нашли снятием
    # покрытия. Сойтись на нём они обязаны так же, как на обычном.
    "неразобранная дата": ryad_dat([
        ("2026-08-01", 140.0), ("2026-08-02", 141.0), ("2026-08-03", 142.0),
        ("2026-08-04", 143.0), ("2026-08-05", 144.0), ("2026-08-06", 145.0),
        ("2026-08-07", 146.0), ("2026-08-10", 147.0), ("2026-08-11", 148.0),
        ("позавчера", 150.0),
    ]),
