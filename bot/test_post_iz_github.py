...
# Настоящие сроки снимаем ДО подмены: ниже пауза обнуляется, чтобы набор
# не ждал минутами, и проверка сроков мерила бы тогда сама себя.
_SROKI_POPYTOK = post_iz_github.POPYTOK
_SROKI_ZHDAT = post_iz_github.ZHDAT_SEKUND
_SROKI_PAUZA = post_iz_github.PAUZA_SEKUND
...
proverka("ожидание не срезано",
         _SROKI_ZHDAT >= 30 and _SROKI_POPYTOK >= 2,
         "холодный старт Render — до минуты; короткий срок даст два поста")

# Отдельно от предыдущей: там про ДОЛГИЙ ответ, здесь про БЫСТРЫЙ отказ.
# Перезапуск сервиса Render отвечает 503 мгновенно, и опрос без пауз
# уложился бы в секунду — целиком внутрь окна перезапуска.
_ves_opros = (_SROKI_POPYTOK - 1) * _SROKI_PAUZA
proverka("быстрый отказ не проглатывается за секунду", _ves_opros >= 120,
         "весь опрос занимает %d с — окно перезапуска Render длиннее"
         % _ves_opros)
...
# ── 12. Правило «молчит ли канал» ────────────────────────────────────
#
# По нему сторож заводит задачу и шлёт письмо. Правило, которое краснеет
# зря, обесценивает и себя, и все остальные письма сторожа, — а проверить
# его на живом канале нельзя: нужный случай бывает раз в месяц и длится
# девять дней.

_KURS_NEDELI = ["2026-08-26", "2026-08-27", "2026-08-28"]

proverka("освещённый курс — зелёное",
         tishina.sudit("2026-08-28", _KURS_NEDELI, 9)[0] == tishina.ZELYONOE)

# ВЫХОДНЫЕ. ЦБ не публикует, канал молчит правильно, и счётчик дней
# молчания краснел бы здесь каждую субботу.
proverka("выходные не краснеют",
         tishina.sudit("2026-08-28", _KURS_NEDELI, 20)[0] == tishina.ZELYONOE,
         "пост в пятницу, новых публикаций ЦБ нет — молчать правильно")

# ПРАЗДНИК. 1 сентября ЦБ не публиковал, а месячный пост вышел — значит
# последний пост НОВЕЕ последнего курса. Отрицательного отставания быть
# не должно.
proverka("пост свежее курса — зелёное",
         tishina.sudit("2026-09-01", _KURS_NEDELI, 20)[0] == tishina.ZELYONOE)

# СЕГОДНЯШНЕЕ МОЛЧАНИЕ. Утром это ожидание, а не поломка: бот пишет с
# девяти, работа GitHub в 11:00 и 15:00.
_utro = tishina.sudit("2026-08-28", _KURS_NEDELI + ["2026-08-31"], 9)
proverka("одна непокрытая дата утром — жёлтое",
         _utro == (tishina.ZHOLTOE, ["2026-08-31"]), str(_utro))

_vecher = tishina.sudit("2026-08-28", _KURS_NEDELI + ["2026-08-31"], 17)
proverka("та же дата после закрытия последней двери — красное",
         _vecher[0] == tishina.KRASNOE, str(_vecher))

proverka("час закрытия последней двери позже работы GitHub",
         tishina.CHAS_POSLEDNEY_DVERI > 15,
         "работа пробует в 15:00 — краснеть раньше значит краснеть зря")

# ГРАНИЦА ЧАСА. Ровно та ошибка, которая живёт годами и находится в один
# конкретный час суток.
proverka("в 15 часов ещё жёлтое",
         tishina.sudit("2026-08-28", _KURS_NEDELI + ["2026-08-31"], 15)[0]
         == tishina.ZHOLTOE)
proverka("в 16 часов уже красное",
         tishina.sudit("2026-08-28", _KURS_NEDELI + ["2026-08-31"], 16)[0]
         == tishina.KRASNOE)

# ДВА ДНЯ И БОЛЬШЕ — красное в любой час. Сторож бегает в 8:30 утра, и
# правило, щадящее утро, промолчало бы ровно тогда, когда его читают.
_dva = tishina.sudit("2026-08-28", _KURS_NEDELI + ["2026-08-31", "2026-09-01"], 8)
proverka("два непокрытых дня краснеют даже утром",
         _dva[0] == tishina.KRASNOE, str(_dva))

# ДЕВЯТЬ ДНЕЙ МОЛЧАНИЯ — то, что уже случилось и осталось незамеченным.
_devyat = tishina.sudit("2026-08-21",
                        ["2026-08-26", "2026-08-27", "2026-08-28",
                         "2026-09-02", "2026-09-03"], 8)
proverka("падение Render на девять дней краснеет",
         _devyat[0] == tishina.KRASNOE and len(_devyat[1]) == 5, str(_devyat))

# НЕ С ЧЕМ СРАВНИТЬ — не то же самое, что «всё хорошо». Та же разница,
# что между «записи нет» и «прочитать не удалось» в хранилище.
proverka("без дат курса — жёлтое, а не зелёное",
         tishina.sudit("2026-08-28", [], 9)[0] == tishina.ZHOLTOE,
         "молчащий канал, объявленный исправным, — худшее из состояний")

proverka("пустой канал: не освещено всё",
         tishina.ne_osveshcheno(None, _KURS_NEDELI) == _KURS_NEDELI)

# СМЕНА ГОДА. Даты сравниваются как строки, и в ISO это то же самое, что
# сравнение дат, — но проверить надо, а не поверить.
proverka("отставание видно через смену года",
         tishina.sudit("2025-12-31", ["2026-01-05", "2026-01-06"], 9)[0]
         == tishina.KRASNOE)
...
# ── 14. Сторож доживает до красного ──────────────────────────────────
#
# Правило про молчащий канал щадит утро: одно непокрытое число до
# четырёх дня — ожидание, а не поломка, потому что работа GitHub пробует
# в 11:00 и 15:00. Значит красная его половина может сработать ТОЛЬКО у
# того, кто смотрит после четырёх.
#
# Сторож, бегающий одним утренним обходом, до неё не доживал бы никогда:
# он узнавал бы о молчании следующим утром, сутки спустя, и половина
# правила была бы написана впустую. Проверка связывает расписание с
# порогом — тронешь одно, покраснеет здесь.

_storozh = os.path.join(KORNI, ".github", "workflows", "storozh.yml")
proverka("сторож на месте", os.path.exists(_storozh), _storozh)

if os.path.exists(_storozh):
    _yml_st = io.open(_storozh, encoding="utf-8").read()
    _delo_st = "\n".join(x for x in _yml_st.splitlines()
                         if not x.strip().startswith("#"))

    _chasy_st = []
    for _stroka in re.findall(r"cron:\s*'([^']+)'", _delo_st):
        _polya = _stroka.split()
        if len(_polya) >= 2:
            # Ташкент — UTC+5. Сдвигаем сразу: сравнивать час из cron с
            # порогом, заданным по Ташкенту, значит сравнивать разное.
            _chasy_st += [(int(c) + 5) % 24
                          for c in _polya[1].split(",") if c.isdigit()]

    proverka("расписание сторожа задано", bool(_chasy_st), _delo_st[:200])
    proverka("сторож смотрит и после закрытия последней двери",
             any(c >= tishina.CHAS_POSLEDNEY_DVERI for c in _chasy_st),
             "обходы по Ташкенту: %s, а красное начинается с %d:00 — "
             "красная половина правила про молчащий канал не сработает "
             "никогда" % (sorted(_chasy_st), tishina.CHAS_POSLEDNEY_DVERI))
    proverka("сторож смотрит и утром",
             any(c < 12 for c in _chasy_st),
             "обходы по Ташкенту: %s — вчерашнее молчание надо заметить "
             "до начала дня" % sorted(_chasy_st))
