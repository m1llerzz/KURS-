...
NASTROYKI = [
    ("DATABASE_URL", "подписчики сотрутся при первом же перезапуске, события "
                     "не пишутся, а ПОСТЫ В КАНАЛ НЕ ВЫХОДЯТ ВОВСЕ: без базы "
                     "отметка о публикации не переживает перезапуск, и один "
                     "пост уходит в канал десятки раз — проверено 16 августа, "
                     "семнадцать копий подряд"),
