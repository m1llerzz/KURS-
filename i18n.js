...
window.I18N = (function () {
  ...
  const TEKSTY = {
    ...
    uz: {
      ...
      'src.upd':         'Rasmiy kursni Markaziy bankdan bevosita olamiz, servis kurslari kuniga bir necha marta yangilanadi. Markaziy bank kursi — {d} holatiga',
    ...
    ru: {
      ...
      // Здесь смешивались две разные вещи: ДАННЫЕ пересобираются часто,
      // а КУРС относится к тому дню, когда его опубликовал ЦБ. Стояло
      // «обновляется ежечасно, последний раз 14.08» — и это читалось как
      // «два дня не обновлялось», хотя всё работало исправно.
      //
      // 31 августа строку пришлось поправить второй раз, и по той же
      // причине: обещанная частота перестала быть правдой. Сбор
      // ежечасно делал бот, бот приостановлен, а данные теперь
      // пересобирает GitHub несколько раз в сутки. Официальный курс при
      // этом приложение спрашивает у ЦБ само, при каждом открытии, — и
      // это единственная частота, за которую мы отвечаем сами.
      'src.upd':         'Официальный курс берём прямо у ЦБ, курсы сервисов обновляются несколько раз в сутки. Курс ЦБ — на {d}',
