...
# Когда опрос Telegram в последний раз получил ответ.
#
# Зачем это отдельно от всего остального. Разговор с людьми и сбор курсов
# живут в РАЗНЫХ потоках, и умереть могут порознь. Если опрос отвалится
# насовсем, снаружи всё останется зелёным: курсы обновляются, /api/rates
# отвечает, страница живости работает, — а бот при этом молчит каждому,
# кто ему напишет. Молчаливая деградация в этом продукте не допускается
# нигде, и здесь она стоила бы всех новых людей разом.
_opros = {"otvetil": 0}
...
# Кто ведёт канал. Спрашивается у Telegram и держится час: список
# администраторов меняется раз в жизни, а дёргать чужой сервер на каждое
# сообщение незачем.
_admin_kanala = {"kogda": 0.0, "sozdatel": "", "vse": set()}


def upravlyayut_kanalom():
    """(создатель, множество всех админов) канала. Спрашивается у Telegram.

    Зачем это вместо переменных окружения. Узкое место продукта — не код
    и не деньги, а пять минут человека, у которого их нет: переменные
    стоят незаданными вторые сутки, и вместе с ними стоят служебные
    команды и еженедельная сводка. А ответ на вопрос «кто здесь свой»
    Telegram знает и так: свои — это те, кого Семён сам назначил
    администраторами канала.

    Переменные при этом не отменяются: заданное всегда сильнее
    выведенного. Это на случай, когда свой человек каналом не управляет.
    """
    teper = time.time()
    if teper - _admin_kanala["kogda"] < 3600:
        return _admin_kanala["sozdatel"], _admin_kanala["vse"]

    kanal = os.environ.get("CHANNEL_ID", "").strip()
    if not kanal:
        return "", set()

    otvet = vyzov("getChatAdministrators", {"chat_id": kanal})
    if not otvet or not otvet.get("ok"):
        # Не ответило — не запоминаем пустоту надолго: через минуту
        # спросим снова. Иначе один сбой сети выключил бы служебные
        # команды на час.
        _admin_kanala["kogda"] = teper - 3540
        return _admin_kanala["sozdatel"], _admin_kanala["vse"]

    sozdatel, vse = "", set()
    for zapis in otvet.get("result") or []:
        chelovek = (zapis or {}).get("user") or {}
        # Бота в списке админов не считаем своим: он там всегда, и с
        # самим собой в служебных командах разговаривать некому.
        if not chelovek.get("id") or chelovek.get("is_bot"):
            continue
        vse.add(str(chelovek["id"]))
        if zapis.get("status") == "creator":
            sozdatel = str(chelovek["id"])

    _admin_kanala.update({"kogda": teper, "sozdatel": sozdatel, "vse": vse})
    return sozdatel, vse
...
def svoi(chat_id):
    ...
    Свои — это, во-первых, перечисленные в `SVOI` и `ADMIN_CHAT_ID`, а
    во-вторых, администраторы канала: право публиковать в канале мы
    доверили им сами, и оно выше права получить готовый текст для чата.

    Никого не нашлось — служебных команд нет ни у кого, включая
    ...
    if str(chat_id) in razresheno:
        return True

    return str(chat_id) in upravlyayut_kanalom()[1]
...
def svodka_dlya_svoih():
    ...
    Адрес берётся из ADMIN_CHAT_ID, а если его нет — у СОЗДАТЕЛЯ канала:
    сводку читает тот, кто ведёт проект, и это он и есть. Только
    создатель, не любой администратор: в сводке лежит разбивка «откуда
    пришли», то есть карта посева, собранная неделями и руками.

    Не нашлось ни того, ни другого — молчим и не мешаем.
    """
    admin = os.environ.get("ADMIN_CHAT_ID", "").strip()
    if not admin:
        admin = upravlyayut_kanalom()[0]
    if not admin:
        return
...
def chasovoy_uvedomleniy():
    ...
    while True:
        try:
            ...
            # того, обновился ли курс в этот день.
            # Прежде чем решать, публиковать ли, забираем отметки, которые
            # мог поставить второй публикатор — работа GitHub
            # (`post_iz_github.py`). Она пишет в канал, когда Render
            # молчит, и отмечает сделанное у Telegram; наши отметки лежат
            # в Postgres. Не заглянув туда, бот честно прочтёт свою пустую
            # базу и повторит пост, который уже вышел утром.
            #
            # Раз в сутки — и именно ЗДЕСЬ, а не в первом попавшемся
            # проходе. Проход в час ночи забрал бы отметки, которых ещё
            # нет, объявил бы день сделанным и оставил бы полдень без
            # проверки: работа публикует позже. Поэтому первый проход с
            # девяти утра — сразу перед первой же попыткой поста.
            #
            # Только с базой: без неё бот берёт отметки из той же памяти у
            # Telegram, что и работа, и переносить их некуда и незачем.
            if (teper.hour >= 9 and otmetki_zabrali != den
                    and hranilishche.na_postgres()):
                otmetki_zabrali = den
                hranilishche.perenesti_otmetki(
    ...
    def do_GET(self):
        ...
        if put == "/api/stats":
            ...
            otvet = {
                ...
                # Сколько секунд назад Telegram ответил на опрос. Отвечает
                # он не реже раза в 50 секунд — столько держится длинный
                # запрос, — так что число заметно больше минуты означает,
                # что бот перестал слышать людей. Ноль — опрос ещё ни разу
                # не отвечал с запуска.
                "opros_sekund_nazad": (
                    int(time.time() - _opros["otvetil"])
                    if _opros["otvetil"] else None),
            }
            ...
            # встало». А заливка на бесплатном тарифе доезжает не всегда
            # и не сразу, и проверять боевое, не зная его версии, значит
            # проверять неизвестно что.
            kod = os.environ.get("RENDER_GIT_COMMIT", "").strip()[:7]
            if kod:
                otvet["kod"] = kod

            # Скольким людям доступны служебные команды. Ноль означает,
            # что /tekst не работает ни у кого, — а без него посев идёт
            # текстами из файла, то есть с позапрошлым курсом. Число, не
            # список: кто именно — не дело открытого адреса.
            otvet["svoih"] = len(upravlyayut_kanalom()[1] | {
                k for imya in ("ADMIN_CHAT_ID", "SVOI")
                for k in os.environ.get(imya, "").replace(" ", "").split(",")
                if k})
...
NASTROYKI = [
    ...
    ("ADMIN_CHAT_ID", "еженедельная сводка пойдёт создателю канала; если и "
                      "его нет — не придёт никому"),
    ("SVOI", "служебные команды остаются у администраторов канала; если "
             "их нет — недоступны никому"),
...
def soobshchit_o_nastroykah():
    ...
    # А заодно — что из этого уже закрыто без переменных. Иначе список
    # пугает сильнее, чем положено: половина того, что в нём названо, уже
    # работает, и человек, прочитав его, идёт делать сделанное.
    sozdatel, admin_kanala = upravlyayut_kanalom()
    if admin_kanala:
        print("[настройки]   а вот это уже работает и без переменных:",
              flush=True)
        print("[настройки]     служебные команды — у %d администраторов "
              "канала" % len(admin_kanala), flush=True)
        if sozdatel:
            print("[настройки]     еженедельная сводка — создателю канала",
                  flush=True)
...
def main():
    ...
    while True:
        ...
        # Ответ пришёл — значит разговор с людьми жив. Отмечаем ПОСЛЕ
        # проверки ответа, а не до запроса: «мы попытались» и «Telegram
        # ответил» — разные вещи, и отметка о попытке скрыла бы ровно то,
        # что мы хотим видеть.
        _opros["otvetil"] = time.time()
