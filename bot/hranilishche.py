...
def otrazit_v_zapasnoy(vyzov, kanal, otmetki):
    """Кладёт копию отметок туда, где их прочитает вторая дверь. Сколько легло.

    Зачем. Отметки бота лежат в Postgres, а работы GitHub
    (`post_iz_github.py`) — на серверах Telegram. Работа не публикует,
    пока Render отвечает, и обычно этого хватает. Но «Render не ответил»
    и «Render мёртв» — не одно и то же: сервис перезапускается, сеть
    моргает, и в такую минуту работа честно решит, что бот молчит, а бот
    к тому времени уже написал. Своей отметки в Postgres она не видит, и
    в канал уходит второй пост.

    Поэтому после каждой публикации бот кладёт копию отметки туда, куда
    работа смотрит. **Это не вторая память, а та же правда, положенная на
    видное место.** Правда по-прежнему одна и живёт в Postgres: сюда
    только пишут, отсюда бот не читает и решений на этом не строит. Если
    копия не легла, хуже, чем было, не станет — работу всё так же
    удерживает вопрос к Render.

    Заодно вторую дверь становится можно проверить вживую, не рискуя
    каналом: запустить работу с заведомо глухим адресом Render и увидеть,
    что она промолчала, потому что прочла отметку бота.
