...
try:
    ...
    try:
        ...
        # Числа ищем ВИДОМ, а не значением, и вот почему.
        #
        # Здесь стояло «"149" in post» и «"140" or "141" in post» — числа
        # дневного поста. Формат меняется по дням: недельный говорит про
        # неделю и месячного среднего не содержит вовсе, а месячный не
        # содержит сегодняшнего курса, если сегодня не лучший и не худший
        # день месяца. То есть обе проверки краснели бы по календарю на
        # исправном продукте — недельная раз в неделю, месячная почти
        # каждое первое число.
        #
        # Требование у поста одно и не зависит от формата: он обязан нести
        # числа, ради которых его читают.
        chisla_kursa = set(re.findall(r"\d+,\d{2}", post))
        proverka("в посте есть числа курса", len(chisla_kursa) >= 3,
                 "нашлось: %s" % (sorted(chisla_kursa) or "ничего"))
...
# Возраст считается ОТ ТАШКЕНТСКОГО дня, а не от UTC. Даты на странице и
# на обложке ставит сборщик по дате курса, то есть по Ташкенту, и с семи
# вечера по UTC там уже завтра: считая от UTC, бот получал возраст «минус
# один день» и красное на исправной странице пять часов в сутки.
#
# Момент задаём сами — иначе поймать это можно было бы только в те самые
# пять часов, а проверка, которая работает не всегда, не проверка.
_vecher_utc = bot.datetime(2026, 8, 16, 21, 0, tzinfo=bot.timezone.utc)
proverka("вечер по UTC — это уже завтрашний день Ташкента",
         bot._segodnya_v_tashkente(_vecher_utc).strftime("%Y-%m-%d")
         == "2026-08-17",
         bot._segodnya_v_tashkente(_vecher_utc).strftime("%Y-%m-%d"))
_utro_utc = bot.datetime(2026, 8, 17, 6, 0, tzinfo=bot.timezone.utc)
proverka("утром по UTC день тот же",
         bot._segodnya_v_tashkente(_utro_utc).strftime("%Y-%m-%d")
         == "2026-08-17")
proverka("у ташкентского дня нет времени",
         bot._segodnya_v_tashkente(_vecher_utc).hour == 0,
         "иначе возраст в днях скакал бы на единицу в зависимости от часа")
