...
if kod == 200 and telo.startswith("{"):
    ...
    # Знает ли вторая дверь, что бот уже опубликовал. Бот кладёт копию
    # отметки туда, куда смотрит работа GitHub; не легла — работа может
    # написать вторым постом, решив по неотвечающему Render, что бот
    # молчит. Защита, которая молча не работает, хуже отсутствующей:
    # именно на неё и рассчитывают.
    if s.get("vtoraya_dver_ne_znaet"):
        preduprezhdenie(
            "вторая дверь не знает о последнем посте",
            "%s — работа GitHub может написать вторым постом, если Render "
            "ей не ответит" % s["vtoraya_dver_ne_znaet"])
